    ...
    <div class="bg-gray-50 py-8 antialiased md:py-12">
        <div class="mx-auto max-w-screen-2xl px-4 2xl:px-0">
            <div class="mb-4 items-end justify-between space-y-4 sm:flex sm:space-y-0 md:mb-8">
                ...
                <ComponentFilter v-model:minimum="minimum" v-model:maximum="maximum" v-model:selected-category-id= "categoryId" :category="categories" 
            ...
            <div class="mb-4 grid gap-4 sm:grid-cols-2 md:mb-8 lg:grid-cols-3 xl:grid-cols-5">
                <ClientOnly>
                    <ItemProduct v-for="item in products" :key="item.id" :item="item"
...
const { data, pending, refresh } = useAsyncData("products", async () => {
    ...
    const response = await getProducts(cookieToken.value, currentPage.value, searchQuery.value, categoryId.value, minimum.value, maximum.value, slug.value);
    if (process.server) {
